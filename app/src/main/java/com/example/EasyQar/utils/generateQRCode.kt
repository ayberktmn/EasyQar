...
fun generateAppLinkQRCode(
    uid: String,
    qrColor: Int,
    backgroundColor: Int
): Bitmap {
    val appPackage = "com.example.qrmycar"
    val deepLink = "mycarapp://user/$uid" // uygulama içi özel URI
    val fallbackLink = "https://play.google.com/store/apps/details?id=$appPackage&referrer=$uid"

    // Eğer Firebase Dynamic Link kullanılmıyorsa direkt fallback kullanılabilir
    val qrContent = fallbackLink
    return generateQRCode(qrContent, qrColor, backgroundColor)
}
